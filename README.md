...
这是一个基于 Web 技术构建的高性能 3D 桥梁模型可视化与模拟系统。项目利用 Three.js 和 @thatopen (Open BIM) 生态组件，实现了在浏览器端加载解析大型 IFC 格式模型，并提供了构件属性查看、模型结构管理以及基础的力学模拟可视化演示功能。系统可通过https://3d-bridge.vercel.app/访问。
...
（ `ifc_converter/` 目录下还另提供了'主桥模型.frag'以供调用简单展示）
